...
export default function App() {
  ...
  const [toast, setToast] = useState("Dados salvos neste dispositivo");
  ...
  const actions = {
    ...
    deleteTransaction: (idToDelete: string) =>
      setData((current) => ({ ...current, transactions: current.transactions.filter((item) => item.id !== idToDelete) })),
    ...
    deleteGoal: (goalId: string) =>
      setData((current) => ({ ...current, goals: current.goals.filter((goal) => goal.id !== goalId) })),
    ...
    deleteFuel: (fuelId: string) =>
      setData((current) => ({ ...current, fuelLogs: current.fuelLogs.filter((item) => item.id !== fuelId) })),
    addRide: (item: Omit<Ride, "id">) =>
      setData((current) => ({ ...current, rides: [{ ...item, id: id("ride") }, ...current.rides] })),
    deleteRide: (rideId: string) =>
      setData((current) => ({ ...current, rides: current.rides.filter((item) => item.id !== rideId) })),
  ...
  async function syncSave() {
    try {
      ...
      setToast("Dados enviados para o Supabase");
    } catch (error) {
      setToast(error instanceof Error ? error.message : "Erro ao salvar no Supabase");
    }
  }
  ...
  async function syncLoad() {
    try {
      ...
      setData(cloudData);
      setToast("Dados carregados do Supabase");
    } catch (error) {
      setToast(error instanceof Error ? error.message : "Erro ao carregar do Supabase");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#090A0F] text-white antialiased">
      ...
      <main className="relative mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center px-3 py-5 sm:px-6">
        <div className="phone-frame">
          <div className="phone-screen">
            ...
            <div className="content-scroll">
              <StatusPill text={toast} cloud={isSupabaseConfigured} />
              {screen === "home" && <HomeScreen data={data} metrics={metrics} setScreen={setScreen} actions={actions} setDraft={setDraft} />}
              {screen === "financas" && <FinancasScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "moto" && <MotoScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "objetivos" && <ObjetivosScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "mais" && <MaisScreen setScreen={setScreen} />}
              {screen === "planejamento" && <PlanejamentoScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "abastecimentos" && <AbastecimentosScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "previsor" && <PrevisorScreen data={data} />}
              {screen === "roles" && <RolesScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "calendario" && <CalendarioScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "insights" && <InsightsScreen data={data} metrics={metrics} />}
              {screen === "conquistas" && <ConquistasScreen data={data} metrics={metrics} actions={actions} setDraft={setDraft} />}
              {screen === "configuracoes" && (
                <ConfiguracoesScreen
                  ...
                  syncLoad={syncLoad}
                  reset={() => {
                    setData(resetLocalData());
                    setToast("Dados resetados para o demo");
                  }}
                />
              )}
            </div>
...
function StatusPill({ text, cloud }: { text: string; cloud: boolean }) {
  return (
    <div className="sync-pill mb-4 flex items-center justify-between rounded-2xl border border-white/8 bg-white/5 px-3 py-2 text-[11px] text-white/48">
      <span className="truncate">{text}</span>
      <Badge tone={cloud ? "green" : "yellow"}>{cloud ? "Supabase" : "Offline"}</Badge>
    </div>
  );
}
...
function BottomNav({ screen, setScreen }: { screen: Screen; setScreen: (screen: Screen) => void }) {
  return (
    <nav className="bottom-nav absolute inset-x-0 bottom-0 z-20 px-4 pb-4 pt-3">
...
function HomeScreen({ data, metrics, setScreen, actions, setDraft }: ScreenProps & { setScreen: (screen: Screen) => void }) {
  return (
    <div className="space-y-5">
      <HeroBalance metrics={metrics} />
      <MonthlyCockpit data={data} metrics={metrics} />
      <FreedomGrid data={data} metrics={metrics} />
      <div className="responsive-split">
        <DailyLimit metrics={metrics} />
        <MotoSummary data={data} metrics={metrics} />
      </div>
      <QuickCategories budgets={data.budgets} />
      <div className="responsive-split">
        <BillsCard bills={data.bills} actions={actions} setDraft={setDraft} />
        <GoalsPreview goals={data.goals} setScreen={setScreen} />
      </div>
      <AssistantCards metrics={metrics} />
      <SurvivalCard metrics={metrics} />
      <BudgetRemaining budgets={data.budgets} />
      <ExecutiveSummary data={data} metrics={metrics} />
    </div>
  );
}

function HeroBalance({ metrics }: { metrics: Metrics }) {
  return (
    <Card className="hero-card relative overflow-hidden bg-gradient-to-br from-violet via-[#5B2AC9] to-electric p-5 shadow-glow">
...
function MonthlyCockpit({ data, metrics }: { data: AppData; metrics: Metrics }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      <SmallStat icon={Wallet} label="Apos contas" value={brl.format(metrics.afterBills)} tone="blue" />
      <SmallStat icon={Bike} label="Pode rodar" value={`${metrics.possibleKm} km`} tone="violet" />
      <SmallStat icon={Fuel} label="Tanques" value={metrics.tanks.toFixed(1)} tone="green" />
      <SmallStat icon={Target} label="Objetivos" value={brl.format(data.settings.savingsGoal)} tone="blue" />
    </div>
  );
}

function FreedomGrid({ metrics }: { data: AppData; metrics: Metrics }) {
  const items = [
...
function DailyLimit({ metrics }: { metrics: Metrics }) {
  return (
    <Card>
...
function QuickCategories({ budgets }: { budgets: Budget[] }) {
  return (
    <section>
...
function MotoSummary({ data, metrics }: { data: AppData; metrics: Metrics }) {
  return (
    <section>
...
function AssistantCards({ metrics }: { metrics: Metrics }) {
  const items = [
  ...
  return (
    <section>
      ...
      <div className="space-y-3">
        {items.map((item) => <Card key={item} className="p-3"><p className="text-sm leading-5 text-white/70">{item}</p></Card>)}
      </div>
    </section>
  );
}

function SurvivalCard({ metrics }: { metrics: Metrics }) {
  return (
...
function BudgetRemaining({ budgets }: { budgets: Budget[] }) {
  return (
    <section>
...
function ExecutiveSummary({ data, metrics }: { data: AppData; metrics: Metrics }) {
  const topGoal = data.goals[0];
  return (
    <Card accent="green" className="mb-2">
...
function FinancasScreen({ data, metrics, actions, setDraft }: ScreenProps) {
  const chart = data.budgets.map((item) => ({ name: item.category, value: item.spent, fill: item.color }));
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-2">
        <SmallStat icon={Banknote} label="Receitas" value={brl.format(metrics.income)} tone="green" />
        <SmallStat icon={CircleDollarSign} label="Despesas" value={brl.format(metrics.expenses)} tone="red" />
        <SmallStat icon={PiggyBank} label="Economia" value={brl.format(metrics.saved)} tone="blue" />
      </div>
      <Card>
        ...
        <div className="h-56">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={chart} dataKey="value" innerRadius={56} outerRadius={86} paddingAngle={4}>
                {chart.map((entry) => <Cell key={entry.name} fill={entry.fill} />)}
              </Pie>
              <Tooltip formatter={(value) => brl.format(Number(value))} contentStyle={{ background: "#171A23", border: "1px solid rgba(255,255,255,.08)", borderRadius: 16 }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </Card>
            ...
            <div key={budget.id}>
              <div className="mb-2 flex items-center justify-between text-sm">
                ...
                <div className="flex gap-2">
                  <NumberInline value={budget.spent} onChange={(spent) => actions.setData((current) => ({ ...current, budgets: current.budgets.map((item) => item.id === budget.id ? { ...item, spent } : item) }))} />
                  <span className="text-white/35">/</span>
                  <NumberInline value={budget.limit} onChange={(limit) => actions.setData((current) => ({ ...current, budgets: current.budgets.map((item) => item.id === budget.id ? { ...item, limit } : item) }))} />
                </div>
              </div>
              <Progress value={used} tone={used > 85 ? "red" : "violet"} />
            </div>
...
function MotoScreen({ data, metrics, actions }: ScreenProps) {
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <NumberInput label="Ano" value={data.moto.year} onChange={(year) => actions.setData((current) => ({ ...current, moto: { ...current.moto, year } }))} />
            <NumberInput label="Km atual" value={data.moto.odometer} onChange={(odometer) => actions.setData((current) => ({ ...current, moto: { ...current.moto, odometer } }))} />
            <NumberInput label="Km/l" value={data.moto.averageKmL} onChange={(averageKmL) => actions.setData((current) => ({ ...current, moto: { ...current.moto, averageKmL } }))} />
            <NumberInput label="Tanque L" value={data.moto.tankLiters} onChange={(tankLiters) => actions.setData((current) => ({ ...current, moto: { ...current.moto, tankLiters } }))} />
            <NumberInput label="Gasolina" value={data.moto.fuelPrice} onChange={(fuelPrice) => actions.setData((current) => ({ ...current, moto: { ...current.moto, fuelPrice } }))} />
            <NumberInput label="Prox. oleo" value={data.moto.nextOilChangeKm} onChange={(nextOilChangeKm) => actions.setData((current) => ({ ...current, moto: { ...current.moto, nextOilChangeKm } }))} />
          </div>
        </div>
      </Card>
      <MotoSummary data={data} metrics={metrics} />
      <div className="grid grid-cols-2 gap-3">
        <SmallStat icon={Fuel} label="Custo por km" value={brl.format(metrics.costPerKm)} tone="green" />
        <SmallStat icon={Bike} label="Tanque cheio" value={`${metrics.fullTankRange} km`} tone="blue" />
      </div>
    </div>
  );
}

function AbastecimentosScreen({ data, metrics, actions, setDraft }: ScreenProps) {
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-2">
        <SmallStat icon={Bike} label="Media" value={`${data.moto.averageKmL} km/l`} tone="green" />
        <SmallStat icon={Fuel} label="Litro" value={brl.format(data.moto.fuelPrice)} tone="blue" />
        <SmallStat icon={Wallet} label="Mes" value={brl.format(metrics.fuelSpend)} tone="violet" />
      </div>
...
function PrevisorScreen({ data }: { data: AppData }) {
  ...
  return (
    <div className="space-y-5">
      <Card accent="violet">
        <NumberInput label="Valor disponivel" value={available} onChange={setAvailable} />
...
function RolesScreen({ data, metrics, actions, setDraft }: ScreenProps) {
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-2">
        <SmallStat icon={Bike} label="Km" value={String(metrics.rideKm)} tone="blue" />
        <SmallStat icon={CalendarDays} label="Roles" value={String(data.rides.length)} tone="violet" />
        <SmallStat icon={Home} label="Cidades" value={String(new Set(data.rides.map((ride) => ride.city)).size)} tone="green" />
      </div>
...
function CalendarioScreen({ data, setDraft }: ScreenProps) {
  return (
    <Card>
      <div className="flex items-center justify-between"><SectionTitle title="Junho" subtitle="Calendario financeiro" /><button onClick={() => setDraft("event")} className="mini-button"><Plus size={16} /> Evento</button></div>
      <div className="grid grid-cols-7 gap-2 text-center text-xs">
        {Array.from({ length: 30 }).map((_, index) => {
          const day = index + 1;
          const events = data.calendarEvents.filter((item) => new Date(`${item.eventDate}T00:00:00`).getDate() === day);
          return (
            <div key={day} className={cn("min-h-14 rounded-2xl bg-white/5 p-1", events.length > 0 && "ring-1 ring-white/12")}>
              <p className="text-white/55">{day}</p>
              {events.slice(0, 1).map((event) => <p key={event.id} className="mt-1 truncate rounded-full px-1 py-0.5 text-[9px] text-white" style={{ backgroundColor: `${event.color}66` }}>{event.title}</p>)}
            </div>
          );
        })}
      </div>
...
function ObjetivosScreen({ data, actions, setDraft }: ScreenProps) {
  return (
...
function InsightsScreen({ metrics }: { data: AppData; metrics: Metrics }) {
  return (
    <div className="space-y-4">
      {[
      ...
      ].map((item) => <Card key={item}><p className="text-sm leading-6 text-white/70">{item}</p></Card>)}
    </div>
  );
}
...
function ConfiguracoesScreen({ data, actions, syncSave, syncLoad, reset }: ScreenProps & { syncSave: () => void; syncLoad: () => void; reset: () => void }) {
  return (
    <div className="space-y-5">
      <Card className="space-y-3">
        <SectionTitle title="Configuracoes financeiras" />
        <NumberInput label="Salario" value={data.settings.salary} onChange={(salary) => actions.setData((current) => ({ ...current, settings: { ...current.settings, salary } }))} />
        <NumberInput label="Renda extra" value={data.settings.extraIncome} onChange={(extraIncome) => actions.setData((current) => ({ ...current, settings: { ...current.settings, extraIncome } }))} />
        <NumberInput label="Meta de economia" value={data.settings.savingsGoal} onChange={(savingsGoal) => actions.setData((current) => ({ ...current, settings: { ...current.settings, savingsGoal } }))} />
        <NumberInput label="Corte diario em alimentacao" value={data.settings.dailyFoodCut} onChange={(dailyFoodCut) => actions.setData((current) => ({ ...current, settings: { ...current.settings, dailyFoodCut } }))} />
      </Card>
      <Card className="space-y-3">
        <SectionTitle title="Supabase" subtitle={isSupabaseConfigured ? "Conectado pelo .env" : "Preencha .env e reinicie o Vite"} />
        <button onClick={syncSave} className="primary-button"><Save size={18} /> Salvar no Supabase</button>
        <button onClick={syncLoad} className="secondary-button"><RotateCcw size={18} /> Carregar do Supabase</button>
        <button onClick={reset} className="danger-button"><Trash2 size={18} /> Resetar dados demo</button>
      </Card>
    </div>
  );
}
...
function FormForModal({ kind, close, actions }: { kind: DraftKind; close: () => void; actions: Actions }) {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    station: "", liters: "10", odometer: "28500", destination: "", distance: "100", cost: "30", color: "#7C3AED",
  });
  ...
  function submit() {
    ...
    if (kind === "event") actions.addEvent({ title: form.name || "Evento", eventDate: form.date, eventType: form.category, color: form.color });
    close();
  }

  return (
    <div className="space-y-3">
      {["transaction"].includes(kind ?? "") && <><TextInput label="Descricao" value={form.description} onChange={(v) => update("description", v)} /><TextInput label="Categoria" value={form.category} onChange={(v) => update("category", v)} /><SelectInput label="Tipo" value={form.type} options={["expense", "income"]} onChange={(v) => update("type", v)} /></>}
      {["bill", "goal", "event"].includes(kind ?? "") && <TextInput label="Nome" value={form.name} onChange={(v) => update("name", v)} />}
      {kind === "bill" && <><NumberInput label="Dia de vencimento" value={num(form.dueDay)} onChange={(v) => update("dueDay", String(v))} /><SelectInput label="Status" value={form.status} options={["Proximo", "Pago", "Atrasado"]} onChange={(v) => update("status", v)} /></>}
      {kind === "goal" && <><NumberInput label="Valor atual" value={num(form.current)} onChange={(v) => update("current", String(v))} /><NumberInput label="Meta final" value={num(form.target)} onChange={(v) => update("target", String(v))} /></>}
      {kind === "fuel" && <><TextInput label="Posto" value={form.station} onChange={(v) => update("station", v)} /><NumberInput label="Litros" value={num(form.liters)} onChange={(v) => update("liters", String(v))} /><NumberInput label="Quilometragem" value={num(form.odometer)} onChange={(v) => update("odometer", String(v))} /></>}
      {kind === "ride" && <><TextInput label="Destino" value={form.destination} onChange={(v) => update("destination", v)} /><NumberInput label="Distancia km" value={num(form.distance)} onChange={(v) => update("distance", String(v))} /><NumberInput label="Custo" value={num(form.cost)} onChange={(v) => update("cost", String(v))} /></>}
      {kind === "event" && <><TextInput label="Tipo" value={form.category} onChange={(v) => update("category", v)} /><TextInput label="Cor" value={form.color} onChange={(v) => update("color", v)} /></>}
      {["transaction", "bill", "fuel"].includes(kind ?? "") && <NumberInput label="Valor" value={num(form.amount)} onChange={(v) => update("amount", String(v))} />}
      {["transaction", "goal", "fuel", "ride", "event"].includes(kind ?? "") && <TextInput label="Data" value={form.date} onChange={(v) => update("date", v)} type="date" />}
...
type ScreenProps = { data: AppData; metrics: Metrics; actions: Actions; setDraft: (kind: DraftKind) => void };
...
function SmallStat({ icon: Icon, label, value, tone }: { icon: React.ElementType; label: string; value: string; tone: "violet" | "blue" | "green" | "red" }) {
  ...
  return <Card className="metric-card p-3"><div className={cn("mb-3 grid h-9 w-9 place-items-center rounded-2xl", toneClass[tone])}><Icon size={17} /></div><p className="text-[11px] text-white/45">{label}</p><p className="mt-1 text-sm font-semibold leading-tight">{value}</p></Card>;
}
...
function IconButton({ onClick, icon: Icon, label }: { onClick: () => void; icon: React.ElementType; label: string }) {
  return <button onClick={onClick} aria-label={label} className="grid h-8 w-8 place-items-center rounded-xl bg-white/7 text-white/50"><Icon size={15} /></button>;
}

function TextInput({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: string }) {
  return <label className="block text-xs text-white/45">{label}<input type={type} value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-2xl border border-white/8 bg-white/7 px-3 py-3 text-sm text-white outline-none focus:border-violet" /></label>;
}

function NumberInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <TextInput label={label} value={String(value)} onChange={(value) => onChange(num(value))} type="number" />;
}

function SelectInput({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label className="block text-xs text-white/45">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-2xl border border-white/8 bg-[#171A23] px-3 py-3 text-sm text-white outline-none focus:border-violet">{options.map((item) => <option key={item}>{item}</option>)}</select></label>;
}

function NumberInline({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return <input value={value} onChange={(event) => onChange(num(event.target.value))} type="number" className="w-16 rounded-xl border border-white/8 bg-white/7 px-2 py-1 text-right text-xs text-white outline-none" />;
}
